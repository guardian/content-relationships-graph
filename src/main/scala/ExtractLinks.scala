...
object ExtractLinks {
  ...
  def extractLinksFromText(string: String, source: String) : List[Link] = {
    ...
    val hrefs = doc.body().select("[href]").asScala.toList.map(_.attr("href")).map(Link(_,s"${source}Link"))
    val srcs = doc.body().select("[src]").asScala.toList.map(_.attr("src")).map(Link(_,s"${source}Src"))
    hrefs ++ srcs
  }
}
